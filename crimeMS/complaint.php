		...
		<table id="customers">
			...
			<?php if($result->num_rows>0)
			{
				...
				while($row=$result->fetch_assoc())
				{
					...
					$output.='<td>'.$row['aadhar'].'</td>';
					$output.='<td>'.$row['status'].'</td>';
					$output.='<td>'.'<a href="editcomplaint.php?id='.$row['id'].'" class="btn btn-danger btn_sm">Edit</a>'.'</td>';
				...
				{
					echo "Sorry, No Compliant info were found";
				}
				?>
			
			
		</table>
		            <center><a href="addcomplaint.php" class="btn btn-warning btn_sm">Add Complaint</a></center>
